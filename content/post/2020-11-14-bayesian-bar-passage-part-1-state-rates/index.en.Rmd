...
logit2prob <- function(logit) {
  ...
  # function to convert log odds to percentage
...
admiss <- read_csv(glue("{data_dir}Admissions.csv"))  %>%
  ...
  # create squred columns for 50thgpa and lsat, to be used in polynomial model
  # many values are negative, so we must first add the minimum to the numbers, 
  # so all are positive
  mutate(gpa_sq = (abs(min(uggpa50)) + uggpa50 + 0.01)^2,
         lsat_sq = (abs(min(lsat50)) + lsat50 + 0.01)^2) %>%
  select(-calendaryear)
...
bar <- left_join(bar, admiss, by=c('schoolname', 'year')) %>%
  ...
  filter(state != "WI",
        # extreme outlier
        state != "PR",
        # remove observations with NA in any values
        complete.cases(.))
```

```{r}
# create dataset of each state's cumulative pass rate
# by grouping by state and summing takers and passers
state_rates <- bar %>%
  group_by(state) %>%
  summarize(takers = sum(takers),
            passers = sum(passers)) %>%
  mutate(pass_rate = passers / takers)
# find cumulative US pass rate, so it can be added to plots
...
*Technical notes:  I conducted this analysis in R with the [rstanarm](https://mc-stan.org/users/interfaces/rstanarm) package for Bayesian modeling.  The dataset represents law school ABA 509 disclosures, aggregated by [AccessLex](http://analytix.accesslex.org/DataSet).*
...
state_intercept <- stan_glmer(cbind(passers, takers - passers) ~ (1 | state),
                              ...
                              data = state_rates, family = binomial("logit"))
int <- as.data.frame(state_intercept)[[1]]
state_int_trace <- state_intercept %>%
  # extract trace as dataframe
  as.data.frame(.) %>%
  # add intercept to estimates
  mutate_all(funs(!!int + .)) %>%
  # convert to percentage
  mutate_all(funs(logit2prob(.))) %>%
  # first and last rows are global intercept and sigma, not states
  .[3:ncol(.)-1]
colnames(state_int_trace) <- state_rates$state
# find 90% poserior interval
hpi <- as.data.frame(posterior_interval(state_intercept))
# extract first row as intercept
# which will later be added to state values
int_hpi <- head(hpi, 1)
# add intercept to state values
hpi[[1]] <- int_hpi[[1]] + hpi[[1]]
hpi[[2]] <- int_hpi[[2]] + hpi[[2]]
# remove first and last rows
hpi <- hpi[3:nrow(hpi)-1, 1:2]
hpi <- hpi %>%
  # convert to percentage
  mutate_all(funs(logit2prob(.))) %>%
  # add column that is state names
  mutate(state = state_rates$state)
# calculate mean of trace
state_int_summary <- state_int_trace %>%
  summarize_all(mean) %>%
  t(.) %>%
  as.data.frame(.) %>%
  ...
  # join hpi
  full_join(., hpi, by='state') %>%
  # join state pass rates
  full_join(., state_rates, by='state')
# change column names to be more descriptive
colnames(state_int_summary) <- c('mean_pass', 'state', 'lower', 'upper', 
                                 'takers', 'passers', 'real_pass') 
```

```{r fig.height=8, fig.width=8}
# ----- create plot of estimates from model
# order by mean bar passage
state_int_summary <- state_int_summary %>%
    arrange(desc(mean_pass))
# create plot of bayesian estimated pass rates, actual rates, and HPI
state_int_summary %>%
  ggplot(aes(mean_pass, fct_inorder(state), color='Bayesian passage rate')) +
  # add vertical line that is the US pass rate
  geom_vline(xintercept = overall_pass$pass_rate, alpha = .7) +
  # bayesian predicted pass rate
  geom_point(color='indianred2', alpha=.7) +
  # bayesian 95% HPI
  geom_errorbarh(aes(xmax = upper, xmin = lower), color='indianred2') +
  # real pass rate
  geom_point(aes(x=real_pass, color='Real passage rate'), color='dodgerblue2', alpha=.9, size=1) +
  # convert x-axis to percentage
  scale_x_continuous(labels=function(x) paste0(x*100,"%")) +
  labs(x='Bar passage rate',
       y='State') +
  theme_minimal()
```
...
# only use school / state / year combinations with at least 5 bar takers
admissions_mod <- bar %>%
  filter(takers > 4)
...
# create multiple models and then compare them
# best model among this model, model with only mean GPA and LSAT, 
# and polynomial model with mean GPA and LSAT
fit_admissions <- stan_glmer(cbind(passers, takers - passers) ~ poly(uggpa50, 2) + poly(lsat50,2) + (1 | state),
                            prior_intercept = student_t(2, 0, 2), prior = normal(.5, 1), 
                            data = admissions_mod, family = binomial("logit"))
# ---- no need to rerun these sections
# fit_admissions_min <- stan_glmer(cbind(passers, takers - passers) ~ uggpa50 + lsat50 + (1 | state),
#                             prior_intercept = student_t(2, 0, 2), prior = normal(.5, 1),
#                             data = admissions_mod, family = binomial("logit"))
# 
# fit_admissions_poly <- stan_glmer(cbind(passers, takers - passers) ~ poly(uggpa50, 2) + poly(lsat50,2) + (1 | state),
#                             prior_intercept = student_t(2, 0, 2), prior = normal(.5, 1),
#                             data = admissions_mod, family = binomial("logit"))
# 
# compare_models(loo(fit_admissions_full, k_threshold = 0.7),
#                loo(fit_admissions_min, k_threshold = 0.7),
#                loo(fit_admissions_poly, k_threshold = 0.7))
# Complete model diagnostics can be examined by launching the model's shinystan app. 
# To launch the app enter `launch_shinystan(fit_grades)` into the console.
```


```{r}
# create dataframe that is each state's predicted bar passage with median
# lsat and gpa
# create dataframe that includes each state and average admissions factors
# undergraduate gpa and lsat are 0 because we scaled these columns to have a mean of 0
state_admissions_prediction <- data.frame(state = unique(admissions_mod$state),
                                          uggpa25 = 0,
                                          uggpa50 = 0,
                                          uggpa75 = 0,
                                          lsat25 = 0,
                                          ...
                                          passers = 0,
                                          takers = 100)
# create predictions, which will return the number of passers out of 100
yhat_state_admissions <- posterior_predict(fit_admissions, state_admissions_prediction) %>%
  as.data.frame(.)
# change column names to state abbreviations
colnames(yhat_state_admissions) <- unique(admissions_mod$state)
# calculate the mean and 90% HPD of the posterior predictions
hpdi_states <-lapply(yhat_state_admissions, function(x) HPDinterval(mcmc(x), 0.90)[c(1,2)]) %>%
  as.data.frame() %>%
  # make each row a different state, and columns for upper and lower HPDI
  t() %>%
  as.data.frame() %>%
  rename(lower_hpdi = V1,
         upper_hpdi = V2) %>%
  # add column for state, so we can merge on states with mean dataset
  mutate(state = row.names(.))
pred_state_admissions <- yhat_state_admissions %>%
  # calcualte mean number of passers for each state
  summarize_all(mean) %>%
  # make each row a state
  t() %>%
  as.data.frame() %>%
  rename(mean_predicted = V1) %>%
  # add column for state, so we can merge on it
  mutate(state = row.names(.)) %>%
  # merge HPDI dataset with mean dataset
  left_join(hpdi_states, by = 'state') %>%
  # we also want to add state pass rates to the predicted rates dataset for plotting
  left_join(state_int_summary[c('real_pass', 'state')], by = 'state') %>%
  # multiply pass rate by 100 to convert decimal to whole number
  mutate(real_pass = real_pass * 100,
         pass_diff = mean_predicted - real_pass) %>%
...
```{r}
# order states by mean pass rate
state_order <- list(categoryorder = "array",
                    categoryarray = pred_state_admissions$state)
pred_state_admissions %>%
  # convert precentages to integers by multiplying by 100
  plot_ly(x = ~mean_predicted, y = ~state, type = "scatter", mode = "markers",
          mode = list(color = 'lightblue',
                      bgcolor = 'lightblue'),
          hoverinfo = 'text',
          text = ~paste0("State:  ", state,
                        "<br>Predicted Pass:  ", round(mean_predicted, 2), '%',
                        "<br>Real Pass:  ", round(real_pass, 2),'%',
                        "<br>Difference:  ", round(pass_diff, 2),'%'),
         # create legend label
         name = 'Predicted pass rate',
         width = 900, height = 1200) %>%
  # add error bars from hpi rgb(17, 157, 255)
  add_segments(x = ~upper_hpdi, xend = ~lower_hpdi, y = ~state, yend = ~state,
              name = '90% posterior interval',
              line = list(color = 'rgb(17, 157, 255)')) %>%
  # add actual passage rate
  add_trace(x = ~real_pass, y = ~state, type = "scatter", mode = "markers",
           # create legend label
           name = "Real passage rate") %>%
  layout(yaxis = state_order,   # order states by pass rates
         xaxis = list(title = 'Pass Rates (%)', # axis label names
                      ticksuffix = "%"),  # add percentage to axis lables
