...
# NOTE: is not filtering times correctly
nc_stops_veil <- nc_stops %>%
  ...
  group_by(county_name, is_black, is_dark) %>%
  sample_frac(.1)
```
...
prediction_df <- predictors %>%
  data_grid(
    minute = seq_range(minute, n = 15, trim = .1),
    subject_age = 30,
    subject_sex = 'male',
...
all_predictions <- bart_mod$prob.train
county_predictions <- bart_mod$prob.test
```
...
post_dark <- avg_prob_is_black(county_predictions, prediction_df$is_dark, T)
post_light <- avg_prob_is_black(county_predictions, prediction_df$is_dark, F)
post_diff <- post_dark - post_light

median_hdci(post_diff)
...
bart_mod <- bartMachine(
  X = as.data.frame(nc_stops_veil[x_vars]), 
  y = as.factor(nc_stops_veil$is_black),
...
bart_mod_county <- bartc(
  ...
  use.ranef = T,
  group.effects = T, # change to see effects by county
  n.chains = 4
)

write_rds(
  list(overall_mod = bart_mod,
       county_mod = bart_mod_county),
  'mods.rds'
)
```

```{r}
mods <- read_rds('mods.rds')

bart_mod <- mods[[1]]
bart_mod_county <- mods[[2]]

sum <- summary(bart_mod, ci.style = 'hpd')

sum$ci.info

plot_indiv(bart_mod_county)
a <- bart_mod_county[4]
sum <- summary(bart_mod_county)
names(bart_mod_county)
