...
predicted_pass_draws <- bar_mod %>%
  ...
  add_fitted_draws(school_mod_pca_spline_att, n = 200, seed = 938) %>%
...
nc_compare <- bar_mod %>%
  filter(state == 'NC',
         str_detect(schoolname, '^Campbell|^Elon|^North Carolina Central')) %>%
  select(schoolname, state, year, takers, passers, schoolpasspct, PC1, PC2, log_attrit) %>%
  arrange(schoolname, year)

# create predictions
nc_compare_preds <- posterior_epred(school_mod_pca_spline_att, newdata = nc_compare, 
                                   draws = 200, seed = 394)

# create matrix of residuals
nc_compare_resid <- t(nc_compare$schoolpasspct - t(nc_compare_preds))

# for the residuals matrix, each column is a year / school combination
# combine all years for a given school into one column
# use function because we will iterate every year
f_nc_residuals <- function(campbell_resid, elon_resid, central_resid, year) {
  
  data.frame(
    campbell = as.vector(nc_compare_resid[,campbell_resid]),
    elon = as.vector(nc_compare_resid[,elon_resid]),
    central = as.vector(nc_compare_resid[,central_resid])
  ) %>%
    # calculate differences between schools in residuals
    mutate(`Campbell minus Elon`  = campbell - elon,
           `Campbell minus NC Central` = campbell - central,
           `Elon minus NC Central` = elon - central) %>%
    # convert the residual comparisons to long form
    pivot_longer(cols = contains("minus"), names_to = 'comparison', values_to = 'diff') %>%
    mutate(year = !!year)
}

# list of parameters to iterate through
# so residuals can be calculated by year
resid_params <- list(
  campbell_resid = seq(1, 6), 
  elon_resid = seq(7, 12), 
  central_resid = seq(13, 18), 
  year = unique(nc_compare$year)
)

# iterate through each year and calculate difference in residuals
nc_residuals <- pmap_df(resid_params, f_nc_residuals)
```

```{r plotResidCompare, fig.height = 7, fig.width = 4}
# calculate HDI of differences
nc_residuals %>%
  ggplot(aes(x = diff, y = as.factor(year), color = comparison)) +
  stat_pointinterval(size = .4) +
  facet_wrap(~comparison, ncol = 1) +
  geom_vline(xintercept = 0, linetype = 2, alpha = .4) +
  scale_x_continuous(labels = scales::percent_format(accuracy = 1)) +
  scale_color_brewer(type = 'qualitative', palette = 'Set2') +
  labs(title = 'Difference in Residuals Between\nComparative NC Schools',
