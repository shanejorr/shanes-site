...
ppc_plot <- function(mod, num_takers, pass_pct, plot_title) {
  ...
  # create plotloo_cols <- c('model', 'elpd_diff', 'se_diff')
loo_col_names <- c('Model', 'Difference in ELPD', 'Std. Error of Difference')
...
mod_loo_list2 <- map(mod_list2, loo)
...
predicted_pass_draws <- bar_mod %>%
  ...
  add_fitted_draws(school_mod_pca_spline2_att, n = 200, seed = 938) %>%
...
```{r, fig.height = 25, fig.width = 10, fig.cap = "Left plot reveals actual and predicted bar passage rates. Right plot shows residuals: actual minus predicted bar passage rates. The percentage on the right is the probability that the actual bar passage rate is higher than the predicted rate. For almost all schools, the 95% credible intervals cross zero. This highlights that there is little we can learn from the residuals about school-level performance on the bar exam."}
